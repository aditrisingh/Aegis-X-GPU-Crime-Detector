# gpu_trt_fp16_realtime_optimized.py
import os
import time
import gc
import cv2
...
# ---------------- CONFIG ----------------
ENGINE_PATH = "models/mc3_fp16.engine"
...
def load_bundle(joblib_path=JOBLIB_PATH):
    print(f"[INFO] Loading FP16 model bundle from {joblib_path} ...")
    bundle = joblib.load(joblib_path)
    return bundle["model"], bundle["scaler"], bundle["pca"]

def prepare_gpu_pca(scaler, pca, device=DEVICE):
    scaler_mean  = torch.from_numpy(scaler.mean_.astype(np.float16)).to(device)
    scaler_scale = torch.from_numpy(scaler.scale_.astype(np.float16)).to(device)
    pca_mean     = torch.from_numpy(pca.mean_.astype(np.float16)).to(device)
    components   = torch.from_numpy(pca.components_.astype(np.float16)).to(device)
    return scaler_mean, scaler_scale, pca_mean, components

def gpu_preprocess_pca_fp16(X_np, scaler_mean, scaler_scale, pca_mean, components):
    X = torch.from_numpy(X_np.astype(np.float16)).to(DEVICE)
    X_scaled = (X - scaler_mean) / scaler_scale
    X_centered = X_scaled - pca_mean
    X_pca = torch.matmul(X_centered, components.t())
    return X_pca.cpu().numpy()

def preprocess_frame_hwc(frame):
    return cv2.resize(frame, (112, 112), interpolation=cv2.INTER_LINEAR)

def inference_job_fp16(clip_chw_t, model, scaler_mean, scaler_scale, pca_mean, components, mc3_runner):
    features = mc3_runner.extract_features(clip_chw_t)
    feat_batch = features.reshape(1, -1).astype(np.float16, copy=False)
    X_pca = gpu_preprocess_pca_fp16(feat_batch, scaler_mean, scaler_scale, pca_mean, components)
...
def main():
    ...
    scaler_mean, scaler_scale, pca_mean, components = prepare_gpu_pca(scaler, pca)
    ...
    while True:
        ...
        frame_hwc = preprocess_frame_hwc(frame)
        ...
        if frame_buffer.is_full() and frame_count % PROCESS_EVERY == 0:
            if len(futures) < 2 or all(f.done() for f in futures):
                ...
                fut = executor.submit(inference_job_fp16, np.copy(clip_chw_t),
        ...
        cv2.imshow("Crime Detector (TRT FP16)", disp)
